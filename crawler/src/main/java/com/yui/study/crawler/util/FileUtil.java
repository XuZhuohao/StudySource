...
public class FileUtil {
    ...
    public static void zipFileList(String path) {
        File directory = new File(path);
        if (!directory.isDirectory()) {
            throw new RuntimeException("压缩文件不是个目录");
        }
        try (OutputStream os = new FileOutputStream(path + ".zip");
    ...
    private static void compress(File[] sourceFiles, ZipOutputStream zos, String name, boolean KeepDirStructure) {
